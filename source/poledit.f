...
c
   ...
   20    format (/,' The Multipole Editing Facility can Provide :',
     ...
     &           /,4x,'(4) Generate Polarizabilities and Groups',
     &           /,4x,'(5) Multipole Parameters from Horton Output')
...
c     perform dynamic allocation of some global arrays
c
c      if (.not. allocated(mp))  allocate (mp(maxatm))
c      if (.not. allocated(dpx))  allocate (dpx(maxatm))
c      if (.not. allocated(dpy))  allocate (dpy(maxatm))
c      if (.not. allocated(dpz))  allocate (dpz(maxatm))
c      if (.not. allocated(q20))  allocate (q20(maxatm))
c      if (.not. allocated(q21c))  allocate (q21c(maxatm))
c      if (.not. allocated(q21s))  allocate (q21s(maxatm))
c      if (.not. allocated(q22c))  allocate (q22c(maxatm))
c      if (.not. allocated(q22s))  allocate (q22s(maxatm))
c
c     zero out the atomic coordinates and DMA values
c
      do i = 1, maxatm
...
c     try to get a filename from the command line arguments
c
c      call nextarg (dmafile,exist)
c      if (exist) then
c         call basefile (dmafile)
c         call suffix (dmafile,'dma','old')
c         inquire (file=dmafile,exist=exist)
c      end if
c
...
c
 ...
 30      format (a120)
         if (i .ne. 0) then
            read (record,*) rpole(1,i),rpole(2,i),rpole(3,i)
c insert read multipole here
            print *,"rpoles",rpole(1,i),rpole(2,i),rpole(3,i)
         end if
